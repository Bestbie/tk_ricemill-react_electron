...
const DebtorModel = ({ show, onClose, sale, weighTickets }) => {
  if (!show || !sale) return null;

  const ticket = weighTickets.find((w) => w.id === sale.ticket_id);
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center z-50">
      <div className="bg-white border border-gray-200 p-6 rounded-xl shadow-lg hover:shadow-xl transition w-96 relative">
        ...
        <h2 className="text-lg font-bold mb-4">รายละเอียดใบสั่งขาย</h2>
        <div>เลขที่ใบสั่งขาย: {sale.invoice_number}</div>
        <div>เลขที่ใบชั่ง: {ticket?.ticket_number}</div>
        <div>ทะเบียนรถ: {ticket?.car_plate}</div>
        <div>สินค้า: {ticket?.product}</div>
        <div>สถานะ: {sale.status}</div>
        <div>ยอดรวม: {(sale.total_amount ?? 0).toLocaleString()}</div>
      </div>
    </div>
  );
};
